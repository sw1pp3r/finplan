...
Текущая навигация: **Дашборд** (`/`) · **Баланс** (`/balance`, бывш. Снимок; счета + снимки) · **Доходы** (`/income`, одна лента `inflows`: `expected`→`received`, пайплайн «Ожидается») · **Расходы** (`/expenses`, обязательства + breakeven и «Ежемесячные расходы»; разовые поддерживают частичную оплату) · **Мечты** (`/wishes`, список + `?view=board` Доска; желания не влияют на прогноз до promote) · **Курс** (`/course`) · **Сервисы** (`/services`; обе песочницы на прогноз НЕ влияют) · **Настройки** (`/settings`, профиль/прогноз/курсы/справочники + дубль управления счетами). `finplan-show-course=0` скрывает только пункт **Курс** и редиректит `/course` на `/services`; **Сервисы** остаются доступными. Старые `/snapshot`, `/plans`, `/board`, `/more/course`, `/more/services` редиректятся на новые маршруты. Доска: full-bleed 12-колоночный грид, `RHYTHM` или ручной `card_size` `small`/`square`/`tall`/`wide`/`large`, Geist вместо Playfair, картинки задаются ссылкой или upload и сохраняются в `/wish-images/...` через `app/images.py` (download+save+SSRF-гард).
...
Полный редизайн UI уже частично вошёл в React: боковая навигация, новые имена разделов, нейтральные токены, тёмная тема, полноэкранный onboarding wizard, демо-персона Артём, `Мечты` как список/доска, отдельные пункты `Курс` и `Сервисы`. Дизайн **сделан** в Codex.ai/design (через скилл `auteur`): 8 экранов-макетов лежат в **`designs/*.html`** (источник истины по визуалу). Полная visual parity по всем макетам и browser-матрице ещё не закрыта:
...
Ключевые решения: навигация **Дашборд · Баланс · Доходы · Расходы · Мечты · Курс · Сервисы · Настройки**; нейтральная zinc/чёрная палитра + тёмная тема; график со сценариями/подушкой/минус-зоной + дропдаун периода; онбординг = демо-витрина + мастер 5 шагов; демо-персона = AI Builder Артём. Тон профессиональный, без инфантильности.
